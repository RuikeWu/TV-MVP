% R is the input return matrix, dimensions times sample size
% k is the the folds
...
function [lambda,All_d] = CV_for_spcov(R,k,tau)

    randn('state',12345);rand('state',1234567); 
    P0 = [0.05:0.05:1.5,10:10:100,150:50:1000,1250:250:6000];
    All_d = []; 
    rand_rank = randperm(size(R',1)); %generate random number without replications
    subnum = floor(size(R,2)/k);
    for i =1:length(P0)
            P = P0(i);
            Diff = 0;
            P = P*(ones(size(R,1)) - eye(size(R,1)));
            tau = tau;
            for  j = 1 : k
                testing_data = R(:,rand_rank((j-1)*subnum+1:j*subnum));
                training_data = R;
            ...
            All_d(i) = Diff/ k;
            if length(nonzeros(sp_cov)) == size(R,1)
                break;
            end
    end
    [~,loc] = max(All_d);
